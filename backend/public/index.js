// API Configuration
const API_BASE_URL = "https://ozodbek-0o94.onrender.com/api";
...
function showPage(pageId) {
  ...
  if (pageId === "home") {
    document
      .querySelector('.nav-links a[onclick*="home"]')
      .classList.add("active");
  } else if (pageId === "services") {
    document
      .querySelector('.nav-links a[onclick*="services"]')
      .classList.add("active");
  } else if (pageId === "contact") {
    document
      .querySelector('.nav-links a[onclick*="contact"]')
      .classList.add("active");
  }
...
function updateAuthUI() {
  ...
  if (currentUser) {
    ...
    document.getElementById(
      "userWelcome"
    ).textContent = `Добро пожаловать, ${currentUser.name}!`;
...
function updateRequestsTable(contacts) {
  ...
  contacts.forEach((contact) => {
    ...
    row.innerHTML = `
                    ...
                    <td><span class="status-badge status-${contact.status.replace(
                      "_",
                      ""
                    )}">${getStatusText(contact.status)}</span></td>
                    <td>${new Date(contact.createdAt).toLocaleDateString()}</td>
                    <td>
                        <button class="btn btn-ghost" onclick="updateRequestStatus('${
                          contact._id
                        }', 'in_progress')" style="padding: 0.5rem;">
                            <i class="fas fa-play"></i>
                        </button>
                        <button class="btn btn-ghost" onclick="updateRequestStatus('${
                          contact._id
                        }', 'completed')" style="padding: 0.5rem;">
